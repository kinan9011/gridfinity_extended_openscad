...
// Generate exactly the same virtual 42 mm grid used by the custom cup script.
// The alignment values below must match align_grid_x / align_grid_y in that script.
module drawer_full_baseplate() {
  drawer_units = [drawer_size_mm.x / pitch.x, drawer_size_mm.y / pitch.y];

  set_environment(
    width = drawer_units.x,
    depth = drawer_units.y,
